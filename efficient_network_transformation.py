"""
Efficient Sandhill Crane Migration Network Transformation
======================================================

This version filters out zero-crane observations BEFORE aggregation,
making it much faster and more focused on actual migration patterns.
"""
...
class EfficientCraneNetworkBuilder:
    """
    Builds a migration network from sandhill crane observation data.
    Filters out zero-crane observations before aggregation for efficiency.
    """
    ...
    def calculate_distances(self, county_data):
        """
        Calculate distances between all county pairs.
        """
        print("Calculating distances between counties...")
        ...
        for i in range(n_counties):
            for j in range(n_counties):
                if i != j:
                    coords_i = (county_data.iloc[i]['lat'], county_data.iloc[i]['lon'])
                    coords_j = (county_data.iloc[j]['lat'], county_data.iloc[j]['lon'])
    ...
    def create_network_edges(self, county_data, distance_matrix):
        """
    ...
    def build_network(self, parquet_file):
        ...
        """
        print("Building Efficient Sandhill Crane Migration Network...")
        print("=" * 60)
        ...
        # Calculate distances
        distance_matrix = self.calculate_distances(county_data)
        
        # Create network edges
        self.create_network_edges(county_data, distance_matrix)
...
def main():
    ...
    builder = EfficientCraneNetworkBuilder(
